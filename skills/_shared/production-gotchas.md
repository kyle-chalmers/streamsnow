...
| Container app throws from `get_active_session()`, or intermittent cross-viewer corruption | `get_active_session()` is warehouse-only and **not thread-safe** in the shared container process. Container apps use `st.connection("snowflake")` everywhere (source: docs/snowflake-docs.md D2). |
