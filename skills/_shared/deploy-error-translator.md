...
|---|---|---|
| `Insufficient privileges`, `does not exist or not authorized`, `Object ... not authorized` on a schema, table or view the app reads | `<ROLE>` lacks a grant on the schema/object the app queries | `GRANT USAGE ON SCHEMA ...; GRANT SELECT ON ... TO ROLE <ROLE>;` (or `IMPORTED PRIVILEGES` for a shared database); `streamsnow deploy-setup --admin` prints the full grant set. Then re-deploy by re-running the CI job. |
| `Compute pool ... does not exist`, `pool ... not found`, `COMPUTE_POOL` invalid | Container runtime declared in `snowflake.yml` but `<POOL>` was never created, or `<ROLE>` lacks `USAGE` on it | `streamsnow deploy-setup --admin` emits the `CREATE COMPUTE POOL` (never for the pre-provisioned `SYSTEM_COMPUTE_POOL_CPU`) and the `GRANT USAGE ON COMPUTE POOL`. Per-invocation infra op: the owner runs it. |
| `External access integration ... does not exist`, `EAI ... not authorized`, PyPI install / network blocked during image build | `<EAI>` missing or not granted; container can't reach PyPI | Create + grant `<EAI>` to `<ROLE>`; ensure the app's `snowflake.yml` references it. `streamsnow deploy-setup --admin` emits the DDL. |
...
| `pool ... is suspended` / cold-start timeout / health check failed but Bootstrap+Promote succeeded | Container cold start (1–3 min) outran the verify step — app likely shipped fine | Distinguish for the user: deploy created the object; only `Verify` flaked. Re-run `streamsnow verify-deploy <slug>` or wait for warm process — usually not a real failure. |
| `No such option` in the Verify step's service-log finding, or repeated Streamlit start banners (restart loop) | Container crash-loop: the base image passes a launcher flag the app's pinned Streamlit rejects (or startup raises) — the service reports healthy while the app never serves | Raise the app's `streamlit` pin to at least the base image's bundled version (`pyproject.toml`), redeploy. Reproduce with `streamsnow verify-deploy <slug>`. |
| Verify step: `no version-source URI contains '/commits/<sha>'` | The object deployed but points at an old stage path — viewers get stale code | Re-run the deploy job for the current commit; if it recurs, check the stage-copy step actually uploaded to `@<STAGE>/commits/<sha>/`. Reproduce with `streamsnow verify-deploy <slug> --sha <sha>`. |
