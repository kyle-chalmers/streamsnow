"""Branding for Acme Fleet — generated by StreamSnow from brand config.
...
def branded_metric(
    ...
    delta_html = (
        f'<div style="font-size:0.8rem;color:#6b7280;">{html.escape(delta)}</div>'
        if delta
        else ""
    )
    st.markdown(
