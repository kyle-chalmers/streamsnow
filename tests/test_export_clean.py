...
def test_detects_ticket_prefix_and_personal_path(tmp_path):
    ...
    # DENY_PATTERNS names real internal prefixes (`\bPROJ-\d{2,}\b`), so the test
    # needs that prefix to prove PROJ- detection - but this file sits in the
    ...
    assert not res["ok"]
    assert any(m.startswith("PROJ-") for m in matches)
...
def test_detects_source_org_names(tmp_path):
    ...
    for leak in ("built at ExampleOrg", "Example Org internal", "the example-org org"):
        doc = tmp_path / "doc.md"
        doc.write_text(f"prose. {leak}. prose.\n")
        assert not scan_tree(tmp_path)["ok"], leak


def test_detects_source_domain_vocabulary(tmp_path):
