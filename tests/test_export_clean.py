...
def test_detects_proprietary_term(tmp_path):
    (tmp_path / "b.md").write_text("This references Example Org internal systems.")
    res = scan_tree(tmp_path)
    assert not res["ok"]
    assert any("example org" in f["match"] for f in res["findings"])


def test_detects_ticket_prefix_and_personal_path(tmp_path):
    (tmp_path / "c.py").write_text("# tracked in PROJ-1339\nP = '/Users/someone/secret/x'\n")
    res = scan_tree(tmp_path)
